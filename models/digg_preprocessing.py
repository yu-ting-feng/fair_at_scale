...
def extract_network(file):
    ...
    to_remove = (
        reciprocal.reset_index()
        .merge(friends, left_on=[2, 3], right_on=[2, 3])
        .set_index("index")
        .index
    )
...
def extract_cascades(file):
    ...
    start_times = votes.groupby("post")[
        "time"
    ].min()  # --- take into consideration only the starting time of each cascade
    ...
    for i in votes["post"].unique():
        ...
        for post in sub.sort_values(by=["time"]).iterrows():
            s = (
                s + str(post[1]["user"]) + " " + str(post[1]["time"]) + ";"
            )  # ":"+str(post[1]["time"])+","
        s = s[:-1]
